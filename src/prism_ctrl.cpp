...
const float DSTL0829[NUM_DSTL0829][2] = {{0.0000673f, 3.5282f},
                                         {0.0002231f, 3.5282f},
                                         {0.0002374f, 3.5282f},
                                         {0.0003524f, 3.5282f},
                                         {0.0007758f, 3.5282f},
...
void cb(const sensor_msgs::JointState &msg) {
  ...
  if (enF) {
    ...
    // else {
    commandMsg.data =
            T_STALL * (1.0f - fmax(0.0f, fmin(1.0f, v / MAX_ANGV))); // T_STALL / P_PARAM; // -Fm; // Fm + Fsp;
    // }
  } else {
    commandMsg.data = -0.5; // -T_STALL / P_PARAM; // Fsp;
  }

  printf("%.3f\t%.3f\t%.3f\t%.3f\t%d\t%.3f\n", zvel_filt, msg.position[0], msg.position[1], v, last_collided,
         commandMsg.data);
